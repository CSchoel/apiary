...
DOCKER_BUILDKIT=1 docker build "$SCRIPT_DIR" -t "apiary/proxmox-auto-install-assistant:$CONTAINER_VERSION"
...
docker run -it --rm --mount "type=bind,src=$OUT_DIR,dst=$DATA_PATH" "apiary/proxmox-auto-install-assistant:$CONTAINER_VERSION" \
...
docker run --rm --mount "type=bind,src=$OUT_DIR,dst=$DATA_PATH" "apiary/proxmox-auto-install-assistant:$CONTAINER_VERSION" \
